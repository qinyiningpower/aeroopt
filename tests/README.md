...
I built AeroOpt end to end, including the browser frontend, Flask backend, regional result-processing pipeline, and AI assistant integration. I connected vehicle geometry, pressure, and drag outputs to interactive comparison views and case-specific AI explanations across seven vehicle cases.

I implemented the application interfaces, case loading, visualization workflow, and prompts that connect research outputs to the AI assistant. The repository also includes portable startup, request-scoped case selection, input validation, credential configuration, safer text rendering, automated checks, and English documentation. See [my project notes](docs/PROJECT_NOTES.md) for the implementation walkthrough.
...
- [My implementation and project walkthrough](docs/PROJECT_NOTES.md)
...
This is a competition portfolio prototype, not an official Mercedes-Benz product or an endorsed engineering tool. I used the [DrivAerNet collection on Harvard Dataverse](https://dataverse.harvard.edu/dataverse/DrivAerNet) as my dataset source. The exact release and per-case mapping remain to be documented. See [data provenance](docs/DATA_PROVENANCE.md).

No blanket software license is granted in this release; dataset-derived assets remain subject to upstream terms. Third-party rights remain with their respective owners. See [NOTICE](NOTICE).
